...
for pair in \
  ...
  "GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON=GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON:latest" \
...
for svc in $DEPLOY_TARGETS; do
  ...
  ENV_CSV="${ENV_CSV}##MAIL_FROM_NAME=Consignment App"
  ...
  ENV_CSV="${ENV_CSV}##INVENTORY_GOOGLE_SHEET_ID=${INVENTORY_GOOGLE_SHEET_ID:-1tTOzLKp_Ybh3kIuQbzZOv6eTdVLMk-3EyzHcEjMQgk4}"
  ENV_CSV="${ENV_CSV}##INVENTORY_GOOGLE_SHEET_NAME=${INVENTORY_GOOGLE_SHEET_NAME:-AutoFetch}"
